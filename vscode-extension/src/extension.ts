...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('contextlite.enableForProject', enableForProject),
        vscode.commands.registerCommand('contextlite.ingestChatHistory', ingestChatHistory),
        outputChannel,
        statusBarItem
    );
...
async function startContextLite() {
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            `ContextLite started for ${projectInstance.projectName} on port ${projectInstance.port}`
        );
...
// 🎯 INTELLIGENT PORT MANAGEMENT FUNCTIONS
...
async function getOrAssignProjectPort(projectPath: string): Promise<ProjectInstance> {
    const homeDir = os.homedir();
    const registryPath = path.join(homeDir, '.contextlite', 'port_registry.json');
    
    // Load existing registry
    let registry: {[key: string]: ProjectInstance} = {};
    try {
...
async function createProjectConfig(instance: ProjectInstance): Promise<void> {
...
async function enableForProject() {
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            `ContextLite enabled for ${projectName} on port ${projectInstance.port}`,
            'Start Now', 'Configure'
        ).then(selection => {
...
async function ingestChatHistory() {
    ...
    if (!fs.existsSync(contextLiteDir)) {
        const enable = await vscode.window.showInformationMessage(
            'ContextLite not enabled for this project. Enable it first?',
            'Enable', 'Cancel'
        );
        if (enable === 'Enable') {
    ...
    const selection = await vscode.window.showInformationMessage(
        `Found ${chatFiles.length} chat files. Ingest them into ContextLite?`,
        'Ingest All', 'Show Files', 'Cancel'
    );
    ...
    if (!await isPortHealthy(instance.port)) {
        const start = await vscode.window.showInformationMessage(
            'ContextLite is not running. Start it for ingestion?',
            'Start', 'Cancel'
        );
        if (start === 'Start') {
...
function extractDocumentsFromChat(chatData: any, chatFile: string): any[] {
    const documents = [];
