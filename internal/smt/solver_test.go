...
func TestoptimizationSolver_Creation(t *testing.T) {
	// Test optimization system creation with no optimizer path (should work)
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			...
			optimizer: config.optimizerConfig{
				BinaryPath:          "", // No optimizer binary
	...
	solver, err := NewoptimizationSolver(cfg)
	if err != nil {
		t.Fatalf("Failed to create optimization system: %v", err)
	}

	if solver == nil {
		t.Fatal("optimization system is nil")
	}
}

func TestoptimizationSolver_CreationWithInvalidoptimizer(t *testing.T) {
	// Test optimization system creation with invalid optimizer path (should fail)
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			optimizer: config.optimizerConfig{
	...
	_, err := NewoptimizationSolver(cfg)
	if err == nil {
		t.Error("Expected error when optimizer binary is not available")
	}
}

func TestoptimizationSolver_OptimizeSelection_Fallback(t *testing.T) {
	// Test that optimization system falls back to greedy when optimizer is not available
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 5000,
			MaxCandidates:   100,
...
func TestoptimizationSolver_GreedyWeightedSelection(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 5000,
			MaxCandidates:   100,
...
func TestoptimizationSolver_GreedyConstrainedSelection(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 5000,
			MaxCandidates:   100,
	...
	// Since optimizer is not available, should use MMR fallback
	if result.SolverUsed != "mmr-fallback" {
		t.Logf("Expected mmr-fallback, got %s (this is expected without optimizer)", result.SolverUsed)
	}
}

func TestoptimizationSolver_EpsilonConstraintFallback(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 5000,
			MaxCandidates:   100,
	...
	// Should fall back since no optimizer
	if result.SolverUsed == "" {
		t.Error("Solver used should be set")
	}
}

func TestoptimizationSolver_SelectTopCandidates(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
...
func TestoptimizationSolver_ComputeTierMultipliers(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			IntegerScaling: 1000,
		},
...
func TestoptimizationSolver_ComputeObjectiveValue(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			IntegerScaling: 1000,
		},
...
func TestoptimizationSolver_EmptyDocumentList(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 5000,
		},
...
func TestoptimizationSolver_LargeBudget(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 5000,
		},
...
func TestoptimizationSolver_ConvertPairsForoptimizer(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			MaxPairsPerDoc: 5,
		},
	...
	// convertPairsForoptimizer should be called internally
...
func TestoptimizationSolver_LegacyMethods(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			IntegerScaling: 1000,
			ObjectiveStyle: "weighted-sum",
...
func TestoptimizationSolver_SolveWeightedSum(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 100,
			MaxCandidates:   10,
...
func TestoptimizationSolver_SolveLexicographic(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 100,
			MaxCandidates:   10,
...
func TestoptimizationSolver_SolveEpsilonConstraint(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			SolverTimeoutMs: 100,
			MaxCandidates:   10,
	...
	result, err := solver.OptimizeSelection(ctx, docs, 8, 2)
	if err != nil {
		t.Logf("Epsilon budget solving failed (expected if z3 not available): %v", err)
	} else {
		t.Logf("Epsilon budget test completed using %s", result.SolverUsed)
	}
}

func TestoptimizationSolver_GreedyWeightedSelection_Coverage(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
...
func TestoptimizationSolver_GreedyConstrainedSelection_Coverage(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
...
func TestoptimizationSolver_ComputeTierMultipliers_Coverage(t *testing.T) {
	cfg := &config.Config{
		optimization: config.optimizationConfig{
			ObjectiveStyle: "greedy-weighted",
		},
...
func TestoptimizationSolver_AllObjectiveStyles(t *testing.T) {
...
func TestoptimizationSolver_ForceFallback(t *testing.T) {
	// Create config with valid optimizer but ensure we can trigger fallback
	cfg := &config.Config{
		optimization: config.optimizationConfig{
	...
	solver, err := NewoptimizationSolver(cfg)
	if err != nil {
		// Skip test if optimizer not available - this will still test the paths where optimizer isn't available
		t.Skipf("optimizer not available: %v", err) 
	}
	...
	// Should succeed either via optimizer or fallback
	if err != nil {
...
func TestoptimizationSolver_AllFallbackStyles(t *testing.T) {
	// Test all objective styles 
	styles := []string{"weighted-sum", "lexicographic", "epsilon-budget", "greedy-weighted", "greedy-constrained"}
	...
	for _, style := range styles {
		t.Run(style, func(t *testing.T) {
			...
			solver, err := NewoptimizationSolver(cfg)
			if err != nil {
				t.Skipf("optimizer not available for style %s: %v", style, err)
				return
			}
			...
			if err != nil {
				...
				t.Logf("Style %s succeeded using %s", style, result.SolverUsed)
				// Don't require specific solver - could be optimizer or algorithm fallback
			}
		})
	}
}
