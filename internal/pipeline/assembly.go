...
func (p *Pipeline) AssembleContext(ctx context.Context, req *types.AssembleRequest) (*types.QueryResult, error) {
	...
	timings.optimizationSolverMs = int(time.Since(optimizationStart).Milliseconds())
...
func (p *Pipeline) optimizeSelection(ctx context.Context, scoredDocs []types.ScoredDocument, req *types.AssembleRequest) (*optimization.optimizationResult, error) {
	...
	// Create optimization system config
	optimizationConfig := optimization.optimizationConfig{
		TimeoutMs:      req.optimizationTimeoutMs,
		MaxOptGap:      req.MaxOptGap,
		MaxCandidates:  p.config.optimization.MaxCandidates,
		MaxPairsPerDoc: p.config.optimization.MaxPairsPerDoc,
		IntegerScaling: p.config.optimization.IntegerScaling,
		ObjectiveStyle: req.ObjectiveStyle,
		Weights:        *weights,
	}
	
	// Apply defaults for missing request parameters
	if optimizationConfig.TimeoutMs <= 0 {
		optimizationConfig.TimeoutMs = p.config.optimization.SolverTimeoutMs
	}
	if optimizationConfig.MaxOptGap <= 0 {
		optimizationConfig.MaxOptGap = p.config.optimization.MaxOptGap
	}
	if optimizationConfig.ObjectiveStyle == "" {
		optimizationConfig.ObjectiveStyle = p.config.optimization.ObjectiveStyle
	}
	
	// Create solver and optimize
	solver := optimization.NewoptimizationSolver(optimizationConfig)
...
func (p *Pipeline) assembleResult(req *types.AssembleRequest, 
	...
	optimizationMetrics := types.optimizationMetrics{
		...
		OptimalityGap:   optimizationResult.OptimalityGap,
		SolveTimeMs:     optimizationResult.SolveTimeMs,
...
func (p *Pipeline) getWorkspaceWeights(ctx context.Context, workspacePath string) (*optimization.WeightConfig, error) {
	weights, err := p.storage.GetWorkspaceWeights(ctx, workspacePath)
	if err != nil {
		return p.getDefaultWeights(), nil // Use defaults if not found
	}
	
	return &optimization.WeightConfig{
...
func (p *Pipeline) getDefaultWeights() *optimization.WeightConfig {
	return &optimization.WeightConfig{
