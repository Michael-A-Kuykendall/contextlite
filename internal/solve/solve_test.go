...
func TestoptimizerOptimizer_Creation(t *testing.T) {
	// Test optimizer optimizer creation (should work even without optimizer binary)
	optimizer := NewoptimizerOptimizer("", 5000)
	if optimizer == nil {
		t.Fatal("optimizer optimizer is nil")
	}
}

func TestCheckoptimizerAvailable_NoBinary(t *testing.T) {
	// Test optimizer availability check with no binary path
	err := CheckoptimizerAvailable("")
	if err == nil {
		t.Error("Expected error when optimizer binary path is empty")
	}
}
...
func TestoptimizerOptimizer_ParseoptimizerOutputComprehensive(t *testing.T) {
	optimizer := NewoptimizerOptimizer("", 5000)
...
(define-fun x2 () Int
  0)`
		...
		result, err := optimizer.parseoptimizerOutput(output)
		if err != nil {
			t.Fatalf("parseoptimizerOutput failed: %v", err)
		}
