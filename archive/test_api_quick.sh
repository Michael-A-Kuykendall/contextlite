...
# Quick test script to verify optimizer integration and API fields
...
echo "Testing context assembly with optimization..."
RESPONSE=$(curl -s -X POST http://localhost:8080/api/v1/context/assemble \
  ...
  -d '{"query": "authentication systems", "max_documents": 3, "use_optimization": true}')

echo "Full Response:"
echo "$RESPONSE" | head -c 2000

echo -e "\n\nExtracting optimization Metrics:"
echo "$RESPONSE" | grep -o '"optimization_metrics":{[^}]*}' || echo "optimization metrics not found"
