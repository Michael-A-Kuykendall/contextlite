...
func buildDebugoptimizationModel(docs []types.ScoredDocument, pairs []solve.DocumentPair, maxTokens int, maxDocs int) string {
	...
	// Linking budgets: y_ij ≤ x_i, y_ij ≤ x_j, x_i + x_j - y_ij ≤ 1
	if len(pairs) > 0 {
		sb.WriteString("; Linking budgets\n")
		for _, pair := range pairs {
			varName := fmt.Sprintf("y%d_%d", pair.DocI, pair.DocJ)
...
func main() {
	...
	optimizationModel := buildDebugoptimizationModel(docs, pairs, 200, 2)
	
	fmt.Println("Generated optimization-LIB2 Model:")
	fmt.Println("========================")
	fmt.Println(optimizationModel)
	
	// Now test with optimizer
	z3 := solve.NewoptimizerOptimizer("C:\\ProgramData\\chocolatey\\bin\\z3.exe", 5000)
	...
	if err != nil {
		log.Fatalf("optimizer optimization failed: %v", err)
	}
