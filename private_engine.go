/*
 * ContextLite Private Engine - optimization-Optimized AI Context Engine
...
import (
	...
	"contextlite-private/internal/optimization"
	"contextlite-private/internal/solve"
	"contextlite-private/pkg/types"
)

// PrivateEngine implements the full Advanced context engine
...
func NewPrivateEngine(storage types.StorageInterface, config *types.EngineConfig) (*PrivateEngine, error) {
	...
	optimizationSolver := optimization.NewSolver()
...
// AssembleContext performs Advanced context assembly
func (e *PrivateEngine) AssembleContext(ctx context.Context, request types.ContextRequest) (*types.ContextResult, error) {
	...
	// 3. optimization optimization (proprietary)
	selectedIndices, optimizationMetrics, err := e.optimizationSolver.OptimizeSelection(ctx, scoredCandidates, types.SelectionConstraints{
	...
	if err != nil {
		return nil, fmt.Errorf("optimization optimization failed: %w", err)
	}
	...
	for i, idx := range selectedIndices {
		...
		selectedDocs[i] = types.DocumentReference{
			...
			InclusionReason: "optimization_optimized",
		}
...
func (e *PrivateEngine) GetStats() (*types.EngineStats, error) {
	return &types.EngineStats{
		...
		SolverStats:          e.optimizationSolver.GetStats(),
		MemoryUsageMB:        128.5,
